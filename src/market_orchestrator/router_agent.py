...
class RouterAgent:
    ...
    def route(self, query: str) -> RoutingDecision:
        ...
        if len(matches) >= 2:
            ...
            log.info(f"   Query touches multiple domains, consulting ALL specialists.")
            return RoutingDecision(mode="fallback", confidence=0.0)
...
1. Integrá los datos de TODAS las fuentes disponible en un análisis único
2. Si una fuente devolvió datos, USALOS. No digas que no hay datos si los recibiste.
3. Si hay datos contradictorios, señalalos y explicá por qué
4. Si una fuente específicamente dice "no disponible", simplemente no la menciones
5. Respondé en el mismo idioma de la consulta
6. Mencioná las fuentes: CoinMarketCap para crypto, FRED para macroeconomía
7. Formato: narrativo, ejecutivo, en párrafos cortos

NUNCA digas "no tengo acceso a datos de X" si los datos están en el contexto arriba."""
            
            response = llm.invoke([
                {"role": "system", "content": "Sos un analista financiero senior multi-mercado. Tenés acceso a CoinMarketCap (crypto), FRED (macroeconomía) y el repositorio de acciones."},
