...
HTTP API layer that wraps the existing `MarketOrchestrator` behind a FastAPI application, exposing health checks, chat (single + streaming), conversation CRUD, and reset operations. Coexists with the Streamlit app without modifying it. All endpoints except `/api/health` and `/api/auth/*` require valid Supabase JWT authentication via `Depends(get_current_user)`.
...
The system SHALL provide a liveness check that does not depend on orchestrator readiness and remains publicly accessible without authentication.
